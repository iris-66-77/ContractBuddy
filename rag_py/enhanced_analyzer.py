# -*- coding: utf-8 -*-
"""
增强分析器模块（对应 Node.js 的 enhancedAnalysis.js）

整合向量存储、Embedding服务与条款风险匹配器，
提供合同增强分析、法律知识检索与相似案例搜索功能。
"""

from typing import Any, Dict, List, Optional

from .embedding_service import EmbeddingService
from .pattern_matcher import ClauseMatcher
...
class EnhancedAnalyzer:
    ...
    def __init__(self, vector_store: Optional[VectorStore] = None,
        ...
        Args:
            ...
            embedding_service: Embedding服务实例，默认None。
        """
            ...
            for match in result["patternAnalysis"]["details"]:
                ...
                for res in search_result.get("results", []):
                    result["legalReferences"].append({
                        ...
                        "score": res["score"],
                        "source": res.get("source", "keyword")
                    })

        result["integratedAnalysis"] = self._generate_integrated_analysis(result)
        return result

    def _generate_integrated_analysis(self, result: Dict[str, Any]) -> Dict[str, Any]:
        """
        生成综合分析报告。

        Args:
            result: 包含patternAnalysis和legalReferences的中间结果。

        Returns:
            综合分析字典，包含整体风险等级、风险评分与建议。
        """
        ...
        if pattern_analysis["riskyClauses"] > 0:
            high_risk_count = sum(
                1 for d in pattern_analysis["details"]
                if d["maxRisk"] in ("high", "critical")
            )
        ...
        suggestions = self._generate_suggestions(pattern_analysis, result.get("legalReferences", []))
    ...
    def _generate_suggestions(self, pattern_analysis: Dict[str, Any],
                              legal_references: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
        根据分析结果生成建议列表。

        Args:
            pattern_analysis: 模式分析结果。
            legal_references: 法律依据列表。

        Returns:
            建议列表，每条包含type、content和priority。
        """
        ...
        high_risk_items = [
            d for d in pattern_analysis["details"]
            if d["maxRisk"] in ("high", "critical")
        ]
    ...
    async def add_contract_to_knowledge(self, contract: Dict[str, Any]) -> Dict[str, Any]:
        ...
        Args:
            contract: 合同字典，需包含 originalText、category、riskLevel 等字段。
        ...
        knowledge_doc = {
            ...
            "metadata": {
                ...
                "riskLevel": contract.get("riskLevel", "low"),
    ...
    async def search_similar_contracts(self, text: str, options: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        Args:
            text: 查询文本。
            options: 可选参数，支持 limit、category。
